...
- Environment tested: Local Streamlit app on Windows, launched from the repo
  virtualenv; deployed app access attempted but not fully verifiable from this
  Codex environment.
- App URL: https://card-trackerr.streamlit.app/
...
- The deployed Streamlit URL was attempted first.
- The generic fetch path did not expose the interactive app content, and the
  in-app browser connection was blocked by local AppData permissions before it
  could complete an interactive deployed smoke test.
...
| --- | --- | --- |
| First-run experience | Pass locally / needs deployed verification | Local app opened in single-owner mode and showed Portfolio metrics and empty-state structure. Tyler still needs to verify the deployed clean-browser and Google sign-in path. |
...
- Deployed app could not be interactively verified from this environment, so
  deployed readiness is not fully confirmed.
...
1. Tyler should open https://card-trackerr.streamlit.app/ in a clean browser or
   incognito session.
...
upload, and CSV download on https://card-trackerr.streamlit.app/.
